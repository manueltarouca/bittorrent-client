...
```
    ...
    ┌─────────────────┐         ┌─────────────────┐         ┌─────────────────┐
    │   CLI Interface │         │  Tracker Client │         │  Torrent Parser │
    │ (cmd/bittorrent)│────────▶│ (internal/tracker)│────────▶│(internal/torrent)│
    └─────────────────┘         └─────────────────┘         └─────────────────┘
              │                           │                           │
              │                           ▼                           ▼
              │                  ┌─────────────────┐         ┌─────────────────┐
              │                  │  Peer Discovery │         │ Bencode Parser  │
              │                  │   (HTTP/DHT)    │         │(internal/bencode)│
              │                  └─────────────────┘         └─────────────────┘
              │                           │
              ▼                           ▼
    ┌─────────────────┐         ┌─────────────────┐
    │Download Coordinator│◀──────│  Peer Manager   │
    │(internal/download) │       │(internal/peer)  │
    └─────────────────┘         └─────────────────┘
              │                           │
              │                           │
              ▼                           ▼
    ┌─────────────────┐         ┌─────────────────┐
    │ Piece Manager   │         │ Multiple Peers  │
    │(internal/piece) │         │ (TCP Connections)│
    │                 │         │                 │
    │ ┌─────────────┐ │         │ ┌─────────────┐ │
    │ │ Strategies  │ │         │ │Peer 1 (🌐) │ │
    │ │- Sequential │ │         │ │Peer 2 (🌐) │ │
    ...
    └─────────────────┘         └─────────────────┘
              ...
              ▼                           ▼
    ┌─────────────────┐         ┌─────────────────┐
    │  Disk Manager   │         │ BitTorrent Wire │
    │ (internal/disk) │         │    Protocol     │
    │                 │         │                 │
    │ ┌─────────────┐ │         │ ┌─────────────┐ │
    │ │File I/O     │ │         │ │ Handshake   │ │
    │ │SHA-1 Verify │ │         │ │ Messages    │ │
    │ │Multi-File   │ │         │ │ Keep-Alive  │ │
    │ └─────────────┘ │         │ └─────────────┘ │
    └─────────────────┘         └─────────────────┘
              │
              ▼
    ...
    Data Flow:
    ═════════
    1. CLI parses .torrent file using Bencode Parser
    2. Tracker Client discovers peers from HTTP trackers
    3. Peer Manager establishes connections via BitTorrent protocol
    4. Download Coordinator orchestrates multi-peer downloads
    5. Piece Manager selects pieces using smart strategies
    6. Disk Manager verifies and stores downloaded data
```
